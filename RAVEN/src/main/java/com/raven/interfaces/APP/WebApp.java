...
import com.raven.interfaces.APP.api.*;
...
public final class WebApp {
    ...
    public WebApp(ServerConfig Config, ListenerMode Mode) {
        ...
        ServerApiHandler   = new ServerApi(Config, ServerManager, this::OnEvent);
        ...
        OperatorApiHandler = new OperatorApi(Database, WebLogger, AuthApi, Config.GetAdminUsername());
    ...
    public void Run(String Host, int Port) throws Exception {
        ...
        Router.Register("/api/team/chat/send",          ChatApiHandler::Send);
        Router.Register("/api/team/chat/messages",      ChatApiHandler::History);
        ...
        if (!ApiOnly) Router.RegisterStatic();

        HttpSrv.setExecutor(Executors.newFixedThreadPool(20, Task -> {
        ...
        if (ApiOnly) {
            Logger.Info("TeamClient API listening on http://" + Host + ":" + Port);
        } else {
    ...
    public void Stop() {
        ServerManager.Stop();
        try { if (HttpSrv != null) HttpSrv.stop(1); } catch (Exception Ignored) {}
        HttpSrv = null;
    ...
    private void OnEvent(EventType Type, Map<String, Object> Data) {
        switch (Type) {
            ...
            case ServerStarted -> WebLogger.Add("Listener started on " + Data.get("Host") + ":" + Data.get("Port") + " [" + Data.get("Mode") + "]");
