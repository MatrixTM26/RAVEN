...
public final class PostgresDatabase extends TeamDatabase {
    ...
    public boolean UpdateOperatorPassword(String Username, String PasswordHash) {
        exec("UPDATE tc2_operators SET password_hash=? WHERE username=?", PasswordHash, Username);
        return true;
    }
    ...
    public void UpdateLastSeen(String Username) {
        exec(
            "UPDATE tc2_operators SET last_seen=? WHERE username=?",
    ...
    public String GetLastSeen(String Username) {
        try {
            return query("SELECT last_seen FROM tc2_operators WHERE username=?", Username);
        } catch (Exception E) {
            return "Never";
        }
    }
