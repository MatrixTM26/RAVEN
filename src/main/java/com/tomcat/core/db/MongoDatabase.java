...
public final class MongoDatabase extends TeamDatabase {
    ...
    public void UpdateLastSeen(String Username) {
        try {
            MongoDatabaseRef.getCollection("operators").updateOne(
                new org.bson.Document("username", Username),
                new org.bson.Document("$set", new org.bson.Document("lastSeen",
    ...
    public String GetLastSeen(String Username) {
        try {
            org.bson.Document Doc = MongoDatabaseRef.getCollection("operators")
                .find(new org.bson.Document("username", Username)).first();
            if (Doc != null && Doc.containsKey("lastSeen")) return Doc.getString("lastSeen");
        } catch (Exception E) {
