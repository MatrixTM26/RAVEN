...
public final class SqliteDatabase extends TeamDatabase {
    ...
    private void Migrate() {
        try (Statement St = Conn.createStatement()) {
            try {
                St.executeUpdate("ALTER TABLE Operators ADD COLUMN LastSeen TEXT NOT NULL DEFAULT 'Never'");
    ...
    private void SeedAdmin() throws Exception {
        try (
            PreparedStatement Ps = Conn.prepareStatement(
                "INSERT OR IGNORE INTO Operators (Username,PasswordHash,Role,CreatedAt,LastSeen) VALUES (?,?,?,?,?)"
            )
        ) {
            Ps.setString(1, "admin");
            Ps.setString(2, HashPassword("admin"));
            Ps.setString(3, OperatorRole.SUPER_ADMIN.name());
        ...
        try (
            PreparedStatement Ps = Conn.prepareStatement(
                "UPDATE Operators SET Role=? WHERE Username=? AND Role IN ('ADMIN','VIEWER')"
            )
        ) {
            Ps.setString(1, OperatorRole.SUPER_ADMIN.name());
    ...
    public List<Map<String, Object>> GetOperators() {
        ...
        try (
            ...
            ResultSet Rs = St.executeQuery("SELECT Username,Role,CreatedAt,LastSeen FROM Operators ORDER BY Username")
        ) {
            while (Rs.next()) {
                Map<String, Object> Row = new LinkedHashMap<>();
    ...
    public void UpdateLastSeen(String Username) {
        try (PreparedStatement Ps = Conn.prepareStatement("UPDATE Operators SET LastSeen=? WHERE Username=?")) {
    ...
    public String GetLastSeen(String Username) {
        try (PreparedStatement Ps = Conn.prepareStatement("SELECT LastSeen FROM Operators WHERE Username=?")) {
            Ps.setString(1, Username);
            ResultSet Rs = Ps.executeQuery();
            if (Rs.next()) return Rs.getString("LastSeen");
        } catch (Exception E) {
