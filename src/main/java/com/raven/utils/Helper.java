    ...
    public static void PrintHelp() {
        ...
        System.out.println(
                String.join(
                        ...
                        "",
                        "    LISTENER MODE",
                        "        -M  --multi         Accept all: raw shell, TOMCAT agent, HTTP"
                                + " beacon  [default]",
                        "        -R  --raw           Plain TCP / raw reverse shell only",
                        "            --http          HTTP beacon only",
                        "            --https         HTTPS beacon  (TLS, no client cert)",
                        "            --tls           TOMCAT agent over TLS",
                        "        -T  --mtls          TOMCAT agent over mTLS  (client cert"
                                + " required)",
                        "            --fmtls         Full mTLS on both TCP and HTTPS beacon",
                        "",
                        "    SERVER",
                        "        -S  <host>          Bind host          (default: server.host in"
                                + " server.properties)",
                        "        -p  <port>          Agent listen port  (default: server.port)",
                        "",
                        "    INTERFACE",
                        "        -C  --cli-mode      CLI solo            — no auth, direct",
                        "        -G  --gui-mode      GUI solo            — no auth",
                        "        -W  --web-mode      Web panel           — no auth",
                        "        -TSC                CLI + TeamServer login  — single-operator team"
                                + " mode",
                        "        -TSG                GUI + TeamServer login",
                        "        -TSW                TeamServer web panel    — multi-operator via"
                                + " HTTP (port: teamserver.port)",
                        "        -TC  --teamclient   TeamClient CLI          — connect to -TSW"
                                + " server as operator 2,3,...",
                        "                            -ts <host>  TeamServer host (default:"
                                + " web.host)",
                        "                            -tp <port>  TeamServer port (default:"
                                + " teamserver.port)",
                        "",
                        "    AGENT GENERATION",
                        ...
                        "        -al <lang>          Agent language: java  python  go  bash "
                                + " (default: java)",
                        ...
                        "",
                        "    OTHER",
                        "        -i                  Initialize CA + server certificate",
                        "        -h                  Show this help",
                        "",
                        "    ROLES",
                        ...
                        "",
                        "        # TeamServer Web — operator 1 (starts agent listener + HTTP"
                                + " panel)",
                        "        java -jar raven.jar -TSW --multi -p 4444 -tp 5001",
                        "",
                        "        # TeamClient CLI — operator 2+ (requires -TSW to be running"
                                + " first)",
                        "        java -jar raven.jar -TC -ts 127.0.0.1 -tp 5001",
                        "",
                        "        # TeamServer CLI — single-operator team mode (login, but no HTTP"
                                + " API)",
                        ...
                        "        java -jar raven.jar -a agent01 -ah 10.0.0.1 -ap 4444 -am -ps -al"
                                + " java",
                        "",
