...
public final class PostgresDatabase extends TeamDatabase {
    ...
    private void InitSchema() throws SQLException {
        try (Statement St = Conn.createStatement()) {
            ...
            St.execute(
                    "INSERT INTO tcoperators (username,passwordhash,role) VALUES ('" +
                            Config.GetAdminUsername() + "','" +
                            HashPassword(Config.GetAdminPassword()) + "','" +
                            Config.GetAdminRole() + "') ON CONFLICT (username) DO NOTHING");
    ...
    public void SaveCommandLog(int AgentId, String Operator, String Command, String Output, boolean Success) {
        exec("INSERT INTO tccommands (agentid,operator,command,output,success) VALUES (?,?,?,?,?)",
                AgentId, Operator, Command, Output, Success);
    }

    @Override
    public void SaveSessionEvent(Map<String, Object> Data, String Event) {
        exec("INSERT INTO tcsessions (agentid,hostname,os,username,agentip,event,data) VALUES (?,?,?,?,?,?,?)",
                str(Data, "ID"), str(Data, "Hostname"), str(Data, "OS"),
                str(Data, "User"), str(Data, "AgentIP"), Event, GsonInst.toJson(Data));
    }

    @Override
    public List<Map<String, Object>> GetCommandHistory(int AgentId, int Limit) {
        return AgentId == 0
                ? query("SELECT * FROM tccommands ORDER BY createdat DESC LIMIT ?", Limit)
                : query("SELECT * FROM tccommands WHERE agentid=? ORDER BY createdat DESC LIMIT ?", AgentId, Limit);
    }
    ...
    public void SetAgentNote(int AgentId, String Note) {
        exec("INSERT INTO tcnotes (agentid,note) VALUES (?,?) ON CONFLICT (agentid) DO UPDATE SET note=?,updatedat=NOW()",
                AgentId, Note, Note);
    }
    ...
    public boolean CreateOperator(String Username, String PasswordHash, OperatorRole Role) {
        try {
            exec("INSERT INTO tcoperators (username,passwordhash,role) VALUES (?,?,?)",
    ...
    public boolean ValidateOperator(String Username, String PasswordHash) {
        List<Map<String, Object>> Rows = query(
                "SELECT 1 FROM tcoperators WHERE username=? AND passwordhash=?", Username, PasswordHash);
        return !Rows.isEmpty();
    }
    ...
    public List<Map<String, Object>> GetOperators() {
        List<Map<String, Object>> Raw = query(
                "SELECT username,role,lastseen,createdat FROM tcoperators ORDER BY createdat");
        ...
        for (Map<String, Object> Row : Raw) {
            ...
            M.put("LastSeen",
                    Row.getOrDefault("lastseen", "Never") != null ? Row.getOrDefault("lastseen", "Never") : "Never");
    ...
    public void UpdateLastSeen(String Username) {
        exec("UPDATE tcoperators SET lastseen=? WHERE username=?",
    ...
    public void SaveChatLog(String FromOperator, String ToOperators, String Message) {
        exec("INSERT INTO tcchatlog (fromoperator,tooperators,message) VALUES (?,?,?)",
                FromOperator, ToOperators, Message);
    }
