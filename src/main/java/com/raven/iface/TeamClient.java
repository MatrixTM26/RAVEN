...
public final class TeamClient {
    ...
    private static final String I = "    "; // 4-space indent
    ...
    private boolean Login() {
        ...
        System.out.println();
        System.out.printf(I + "%sTeamServer: %s%s:%d%s%n", AnsiColor.Red, AnsiColor.White, TsHost, TsPort,
                AnsiColor.Reset);
        System.out.println();
        ...
        try {
            ...
            if (Probe.containsKey("Error")) {
                ...
                if (!ErrMsg.toLowerCase().contains("credential") &&
                        !ErrMsg.toLowerCase().contains("required") &&
                        !ErrMsg.isEmpty()) {
                    System.out.printf(I + "%s✘ TeamServer rejected probe: %s%s%n", AnsiColor.Red, ErrMsg,
                            AnsiColor.Reset);
                    return false;
                }
            }
            ...
            System.out.println();
            System.out.printf(I + "  %s# Operator 1 (primary — starts server + web panel)%s%n", AnsiColor.White,
                    AnsiColor.Reset);
            System.out.printf(I + "  java -jar raven.jar -TSW -p 4444 -tp %d%n%n", TsPort);
            System.out.printf(I + "  %s# Operator 2+ (connect to running server)%s%n", AnsiColor.White,
                    AnsiColor.Reset);
        ...
        for (int Try = 0; Try < 3; Try++) {
            try {
                ...
                System.out.printf("%n" + I + "%s✔ Welcome, %s [%s]%s%n", AnsiColor.Green, OperatorName, OperatorRole,
                        AnsiColor.Reset);
                System.out.printf(I + "%sConnected to TeamServer at %s:%d%s%n%n", AnsiColor.White, TsHost, TsPort,
                        AnsiColor.Reset);
                return true;
            } catch (Exception E) {
            ...
            try {
                ...
                switch (Cmd) {
                    ...
                    case "exit", "quit" -> {
                        ...
                        System.out.printf(I + "%s⏻ Disconnected from TeamServer%s%n%n", AnsiColor.White,
                                AnsiColor.Reset);
                    }
                    default ->
                        System.out.printf(I + "%s✘ Unknown: %s — type 'help'%s%n", AnsiColor.Red, Cmd, AnsiColor.Reset);
                }
            } catch (IOException E) {
                break;
            }
    ...
    private void ShowSessions() {
        try {
            ...
            System.out.printf(I + "%s%-5s %-14s %-14s %-16s %-10s %-10s %s%s%n",
                    AnsiColor.Red, "ID", "NAME/CERT", "TYPE", "IP", "OS", "USER", "SESSION-KEY", AnsiColor.Reset);
            System.out.println(I + "─".repeat(95));
            for (Map<String, Object> A : Agents) {
                System.out.printf(I + "%s#%-4s %-14s %-14s %-16s %-10s %-10s %s%s%n",
                        AnsiColor.White,
                        A.getOrDefault("ID", "?"),
                        truncate(A.getOrDefault("AgentName", "?").toString().toUpperCase(), 13),
                        A.getOrDefault("Type", "?"),
                        A.getOrDefault("AgentIP", "?"),
                        truncate(A.getOrDefault("OS", "?").toString(), 9),
                        truncate(A.getOrDefault("User", "?").toString(), 9),
                        A.getOrDefault("SessionKey", "—"),
                        AnsiColor.Reset);
            }
            System.out.println();
        } catch (Exception E) {
    ...
    private void CmdBroadcast(String[] Parts, String Line) {
        ...
        try {
            ...
            Results.forEach((Id, V) -> {
                ...
                System.out.printf(I + "%s[%s] %s%s%s%n",
                        Ok ? AnsiColor.Green : AnsiColor.Red, Id,
                        Ok ? "✔ " : "✘ ", Res.getOrDefault("Output", ""), AnsiColor.Reset);
            });
        } catch (Exception E) {
            Err(E);
        }
    ...
    private void CmdKill(String[] Parts) {
        ...
        try {
            ...
            boolean Ok = Boolean.parseBoolean(R.getOrDefault("Success", "false").toString());
            if (Ok)
                Ok("Session-" + Parts[1] + " terminated");
            else
                Err("Kill failed: " + R.getOrDefault("Error", "?"));
        } catch (Exception E) {
            Err(E);
        }
    }
    ...
    private void CmdDownload(String[] Parts) {
        ...
        try {
            Map<String, Object> Body = Map.of("AgentId", Integer.parseInt(Parts[1]), "Path", Parts[2], "Operator",
                    OperatorName);
    ...
    private void CmdNote(String[] Parts) {
        ...
        try {
            ...
            Map<String, Object> R = Post("/api/agents/note",
                    Map.of("AgentId", Integer.parseInt(Parts[1]), "Note", Note));
            if (Boolean.parseBoolean(R.getOrDefault("Success", "false").toString()))
                Ok("Note saved for Session-" + Parts[1]);
        } catch (Exception E) {
            Err(E);
        }
    }
    ...
    private void CmdGetNote(String[] Parts) {
        ...
        try {
            ...
            Optional<Map<String, Object>> A = Agents.stream()
                    .filter(X -> Integer.parseInt(X.getOrDefault("ID", "-1").toString()) == Id)
                    .findFirst();
            System.out.printf(I + "Note [%d]: %s%s%s%n", Id, AnsiColor.White,
                    A.map(X -> X.getOrDefault("Note", "(none)").toString()).orElse("(session not found)"),
                    AnsiColor.Reset);
        } catch (Exception E) {
            Err(E);
        }
    ...
    private void CmdHistory(String[] Parts) {
        try {
            ...
            System.out.printf(I + "%s%-5s %-12s %-14s %-36s %s%s%n", AnsiColor.Red,
                    "SID", "OPERATOR", "STATUS", "COMMAND", "TIMESTAMP", AnsiColor.Reset);
            ...
            for (Map<String, Object> H : Hist) {
                ...
                if (Cmd.length() > 36)
                    Cmd = Cmd.substring(0, 35) + "…";
                System.out.printf(I + "%s%-5s %-12s %s%-14s%s %-36s %s%s%n",
                        AnsiColor.White,
                        H.getOrDefault("AgentId", "?"),
                        H.getOrDefault("Operator", "?"),
                        Ok ? AnsiColor.Green : AnsiColor.Red, Ok ? "✔ ok" : "✘ fail",
                        AnsiColor.White, Cmd, H.getOrDefault("Timestamp", ""), AnsiColor.Reset);
            }
            System.out.println();
        } catch (Exception E) {
            Err(E);
    ...
    private void CmdListOperators() {
        try {
            ...
            System.out.println();
            System.out.printf(I + "%s%-18s %-10s %-24s %-20s%s%n",
                    AnsiColor.Red, "USERNAME", "ROLE", "PERMISSIONS", "LAST SEEN", AnsiColor.Reset);
            System.out.println(I + "─".repeat(75));
            for (Map<String, Object> Op : Ops) {
                String Role = Op.getOrDefault("Role", "MEMBER").toString();
                String Perm = com.raven.core.db.TeamDatabase.OperatorRole.FromString(Role).PermissionString()
                        .replaceAll("^\\[.*?\\]\\s*", "");
                String Mark = Op.getOrDefault("Username", "").toString().equals(OperatorName) ? " ◀ YOU" : "";
                System.out.printf(I + "%s%-18s %-10s %-24s %-20s%s%s%n",
                        AnsiColor.White,
                        Op.getOrDefault("Username", "?"),
                        Role,
                        "[" + com.raven.core.db.TeamDatabase.OperatorRole.FromString(Role).ShortPerm() + "] " + Perm,
                        Op.getOrDefault("LastSeen", "Never"),
                        Mark, AnsiColor.Reset);
            }
            System.out.println();
        } catch (Exception E) {
            Err(E);
    ...
    private void CmdChat() {
        try {
            ...
            for (Map<String, Object> M : Msgs) {
                ...
                System.out.printf(I + "%s[%s] %s%s%s [%s]: %s%s%n",
                        Mine ? AnsiColor.Green : AnsiColor.White,
                        M.getOrDefault("Time", ""),
                        Mine ? AnsiColor.Green : AnsiColor.Red, From, AnsiColor.Reset,
                        To.equals("all") ? "all" : "→ " + To,
                        M.getOrDefault("Message", ""), AnsiColor.Reset);
            }
            System.out.println();
        } catch (Exception E) {
            Err(E);
    ...
    private void CmdChatHistory() {
        try {
            ...
            for (Map<String, Object> M : Rev) {
                ...
                boolean Mine = From.equals(OperatorName);
                System.out.printf(I + "%s[%s] %s%s%s [%s]: %s%s%n",
                        Mine ? AnsiColor.Green : AnsiColor.White, Ts,
                        Mine ? AnsiColor.Green : AnsiColor.Red, From, AnsiColor.Reset,
                        To.equals("all") ? "all" : "→ " + To,
                        M.getOrDefault("message", ""), AnsiColor.Reset);
            }
    ...
    private void CmdStatus() {
        try {
            ...
            System.out.println();
            System.out.printf(I + "%sStatus   %s%s%s%n", AnsiColor.Red, AnsiColor.Green, R.getOrDefault("Status", "?"),
                    AnsiColor.Reset);
            System.out.printf(I + "%sMode     %s%s%n", AnsiColor.Red, AnsiColor.White, R.getOrDefault("Mode", "?"));
            System.out.printf(I + "%sAddress  %s%s:%s%n", AnsiColor.Red, AnsiColor.White, R.getOrDefault("Host", "?"),
                    R.getOrDefault("Port", "?"));
            ...
            System.out.printf(I + "%sDB       %s%s (%s)%n", AnsiColor.Red, AnsiColor.White,
                    R.getOrDefault("DbOnline", false).toString().equals("true") ? "Connected" : "Offline",
                    R.getOrDefault("DbType", "?"));
            System.out.printf(I + "%sServer   %shttp://%s:%d%s%n%n", AnsiColor.Red, AnsiColor.White, TsHost, TsPort,
                    AnsiColor.Reset);
        } catch (Exception E) {
            Err(E);
        }
    ...
    private void CmdLogs() {
        try {
            ...
            System.out.println();
            Logs.stream().skip(Math.max(0, Logs.size() - 30))
    ...
    private void Prompt() {
        String R = AnsiColor.Red, W = AnsiColor.White, X = AnsiColor.Reset, NL = "\n";
        ...
        System.out.println();
        System.out.printf(I + W + "Connected as: %s[%s]%s  TeamServer: %s%s:%d%s%n%n",
                AnsiColor.Green, OperatorName != null ? OperatorName + " / " + OperatorRole : "—",
                AnsiColor.White, AnsiColor.Red, TsHost, TsPort, X);
    }
