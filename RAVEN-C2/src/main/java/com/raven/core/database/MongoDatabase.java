...
public final class MongoDatabase extends TeamDatabase {
    ...
    private void SeedDefaultAdmin(ServerConfig Config) {
        try {
            ...
            if (ColOperators.find(Filters.eq("username", AdminUser)).first() == null) {
                ColOperators.insertOne(new Document().append("username", AdminUser).append("passwordhash", HashPassword(Config.GetAdminPassword())).append("role", OperatorRole.FromString(Config.GetAdminRole()).name()).append("lastseen", "Never").append("createdat", java.time.LocalDateTime.now().format(com.raven.utils.RavenConstants.TimestampFmt)));
            }
    ...
    public boolean CreateOperator(String Username, String PasswordHash, OperatorRole Role) {
        try {
            ColOperators.insertOne(new Document().append("username", Username).append("passwordhash", PasswordHash).append("role", Role.name()).append("lastseen", "Never").append("createdat", java.time.LocalDateTime.now().format(com.raven.utils.RavenConstants.TimestampFmt)));
            return true;
        } catch (Exception E) {
    ...
    public void UpdateLastSeen(String Username) {
        try {
            ColOperators.updateOne(Filters.eq("username", Username), Updates.set("lastseen", java.time.LocalDateTime.now().format(com.raven.utils.RavenConstants.TimestampFmt)));
        } catch (Exception E) {
    ...
    public void SaveChatLog(String FromOperator, String ToOperators, String Message) {
        try {
            ColChatLogs.insertOne(new Document().append("from_operator", FromOperator).append("to_operators", ToOperators).append("message", Message).append("timestamp", java.time.LocalDateTime.now().format(com.raven.utils.RavenConstants.TimestampFmt)));
        } catch (Exception E) {
