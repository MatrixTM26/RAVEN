...
public final class TerminalRenderer {
    ...
    public String Box(String Title) {
        ...
        String Top = AnsiColor.White + CornerTopLeft + HorizontalLine.repeat(Inner) + CornerTopRight + AnsiColor.Reset;
        String Middle = AnsiColor.White + VerticalLine + Space.repeat(PaddingLeft) + AnsiColor.Green + Title + Space.repeat(PaddingRight) + AnsiColor.White + VerticalLine + AnsiColor.Reset;
        String Bottom = AnsiColor.White + CornerBottomLeft + HorizontalLine.repeat(Inner) + CornerBottomRight + AnsiColor.Reset;
    ...
    public String OutputBox(String Output) {
        ...
        String Top = AnsiColor.Green + CornerTopLeft + OutputBoxLabel + HorizontalLine.repeat(LabelFill) + CornerTopRight + AnsiColor.Reset;
        String Bottom = AnsiColor.Green + CornerBottomLeft + HorizontalLine.repeat(Inner) + CornerBottomRight + AnsiColor.Reset;
        ...
        for (String Line : Output.split(NewLine, -1)) {
            ...
            while (Stripped.length() > LineWidth) {
                Builder.append(Indent(AnsiColor.Green + VerticalLine + Space + AnsiColor.White + Stripped.substring(0, LineWidth) + AnsiColor.Green + Space + VerticalLine + AnsiColor.Reset + NewLine));
                Stripped = Stripped.substring(LineWidth);
            }
            int Padding = Math.max(0, LineWidth - Stripped.length());
            Builder.append(Indent(AnsiColor.Green + VerticalLine + Space + AnsiColor.White + Stripped + Space.repeat(Padding) + AnsiColor.Green + Space + VerticalLine + AnsiColor.Reset + NewLine));
        }
