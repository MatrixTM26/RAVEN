...
public final class ServerManager {
    ...
    public void ShowStatus(String DatabaseState, String DatabaseType) {
        long UptimeSeconds = ServerStartTime != null ? Duration.between(ServerStartTime, Instant.now()).getSeconds() : 0;
        ...
        if (IsTeamServerMode && OperatorName != null) Logger.Custom("  %sOperator  %s%s%n", AnsiColor.Red, AnsiColor.White, OperatorName);
        System.out.println();
    }

    private void HandleEvent(EventType Type, Map<String, Object> Data) {
        switch (Type) {
            case ServerStarted -> LogManager.Add(AnsiColor.White + " server listening on " + Data.get("Host") + ":" + Data.get("Port") + AnsiColor.Reset, true);
            case AgentConnected -> LogManager.Add(AnsiColor.Green + "  [" + Data.get("AgentName") + "] session-" + Data.get("ID") + " key: " + Data.get("SessionKey") + " (" + Data.get("OS") + ")" + AnsiColor.Reset, true);
            case AgentDisconnected -> LogManager.Add(AnsiColor.Red + "  session-" + Data.get("ID") + " disconnected: " + Data.get("Reason") + AnsiColor.Reset, true);
            case AgentRemoved -> LogManager.Add(AnsiColor.White + "  session-" + Data.get("ID") + " removed" + AnsiColor.Reset, false);
            case Error -> LogManager.Add(AnsiColor.Red + "  Error: " + Data.get("Message") + AnsiColor.Reset, true);
        }
    }
