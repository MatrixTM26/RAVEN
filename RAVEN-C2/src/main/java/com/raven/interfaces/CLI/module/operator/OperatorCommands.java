package com.raven.interfaces.CLI.module.operator;
...
public final class OperatorCommands {
    ...
    public boolean CanManage() {
        return !IsTeamServerMode || (OperatorRoleValue != null && OperatorRoleValue.CanManage());
    }

    public boolean CanKick() {
        return !IsTeamServerMode || (OperatorRoleValue != null && OperatorRoleValue.CanKickOperator());
    }

    public boolean CanExecute() {
        return !IsTeamServerMode || (OperatorRoleValue != null && OperatorRoleValue.CanExecute());
    }
    ...
    public boolean Login(BufferedReader Reader) throws IOException {
        ...
        for (int Attempt = 1; Attempt <= 3; Attempt++) {
            ...
            if (!Database.ValidateOperator(Username, TeamDatabase.HashPassword(Password))) {
                Logger.Custom("  %sInvalid credentials - Attempt %d/3%s%n%n",
                    AnsiColor.Red, Attempt, AnsiColor.Reset);
                continue;
            }
    ...
    public void ShowOperators() {
        ...
        System.out.println();
        Logger.Custom("  %s%-18s %-14s %-24s %-20s%s%n",
        ...
        for (Map<String, Object> Operator : Operators) {
            ...
            Logger.Custom("  %s%-18s %-14s %-24s %-20s%s%s%n",
                AnsiColor.White,
                Operator.get("Username"),
                Role.name(),
                "[" + Role.ShortPerm() + "] " + Role.PermissionString().replaceAll("^\\[.*?\\]\\s*", ""),
