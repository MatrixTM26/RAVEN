...
public final class SessionCommands {
    ...
    public void SetServer(RavenServer Server) {
        this.Server = Server;
    }
    ...
    public void ShowSessions() {
        ...
        for (Session ActiveSession : Sessions) {
            ...
            Logger.Custom("  %s#%-4d %-14s %-14s %-16s %-10s %-10s %s%s%n", AnsiColor.White, ActiveSession.GetId(), DisplayName.length() > 14 ? DisplayName.substring(0, 13) + "-" : DisplayName, ActiveSession.GetSessionType().name(), ActiveSession.GetAgentIp(), OsLabel.length() > 10 ? OsLabel.substring(0, 9) + "-" : OsLabel, ActiveSession.GetUser(), ActiveSession.GetSessionKey(), AnsiColor.Reset);
        }
        System.out.println();
    }
    ...
    public void ShowSessionInfo(int SessionId) {
        if (Server == null) {
            Logger.Info("unavailable in cross-process mode");
            return;
        }
    ...
    public void ShowStats() {
        System.out.println(Renderer.Box("SESSION STATISTICS"));
        System.out.println();
        if (Server == null) {
    ...
    public void Execute(int SessionId, String Command) {
        ...
        if (Success) {
            System.out.println(Renderer.OutputBox(Result[1]));
    ...
    public void Broadcast(List<Integer> SessionIds, String Command) {
        ...
        for (Map.Entry<Integer, String[]> Entry : Results.entrySet()) {
            ...
            System.out.println(Renderer.OutputBox(Entry.getValue()[1]));
    ...
    public void BroadcastAll(String Command) {
        if (Server == null) {
            Logger.Warn("unavailable in cross-process mode");
            return;
        }
        ...
        System.out.println(Renderer.Box("BROADCAST-ALL RESULTS"));
        System.out.println();
        for (Map.Entry<Integer, String[]> Entry : Results.entrySet()) {
            boolean Success = Boolean.parseBoolean(Entry.getValue()[0]);
            Logger.Custom("  %ssession-%-3d %s%n", Success ? AnsiColor.Green : AnsiColor.Red, Entry.getKey(), AnsiColor.Reset);
    ...
    public void Interactive(int SessionId) {
        ...
        Logger.Custom("%n  %s[%s%s%s] %sID: %s%d %sUser: %s%s@%s %sOS: %s%s %sArch: %s%s %sIP: %s%s %sType: %s%s %sKey: %s%s%s%n", AnsiColor.Blue, AnsiColor.White, ActiveSession.GetDisplayName(), AnsiColor.Blue, AnsiColor.Blue, AnsiColor.White, SessionId, AnsiColor.Blue, AnsiColor.White, ActiveSession.GetUser(), ActiveSession.GetHostname(), AnsiColor.Blue, AnsiColor.White, ActiveSession.GetOs(), AnsiColor.Blue, AnsiColor.White, ActiveSession.GetArch(), AnsiColor.Blue, AnsiColor.White, ActiveSession.GetAgentIp(), AnsiColor.Blue, AnsiColor.White, ActiveSession.GetSessionType(), AnsiColor.Blue, AnsiColor.White, ActiveSession.GetSessionKey(), AnsiColor.Reset);
        ...
        while (CurrentSessionId == SessionId) {
            try {
                Logger.Custom("%n  %s[%s%s#%d%s%s]%s ~$ %s", AnsiColor.Blue, AnsiColor.White, ActiveSession.GetDisplayName(), SessionId, AnsiColor.Blue, AnsiColor.White, AnsiColor.Blue, AnsiColor.Reset);
    ...
    public void ShowTasksQueue() {
        System.out.println(Renderer.Box("PENDING TASKS"));
        System.out.println();
