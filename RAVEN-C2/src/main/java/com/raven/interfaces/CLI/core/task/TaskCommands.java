...
public final class TaskCommands {
    ...
    public TaskCommands(TeamDatabase Database, TerminalRenderer Renderer, SessionCommands SessionCommands) {
    ...
    public void ShowCommandHistory(int SessionId, int Limit) {
        ...
        Logger.Custom("  %s%-5s %-12s %-8s %-36s %s%s%n", AnsiColor.Red, "SID", "OPERATOR", "STATUS", "COMMAND", "TIMESTAMP", AnsiColor.Reset);
        ...
        for (Map<String, Object> Record : History) {
            ...
            Logger.Custom("  %s%-5s %-12s %s%-8s%s %-36s %s%s%n", AnsiColor.White, Record.getOrDefault("AgentId", "?"), Record.getOrDefault("Operator", "?"), Success ? AnsiColor.Green : AnsiColor.Red, Success ? "ok" : "fail", AnsiColor.White, Command, Record.getOrDefault("Timestamp", ""), AnsiColor.Reset);
        }
        System.out.println();
    }
    ...
    public void GetNote(int SessionId) {
        ...
        Logger.Custom("  Note [session-%d]: %s%s%s%n", SessionId, AnsiColor.White, Note.isEmpty() ? "(empty)" : Note, AnsiColor.Reset);
    }
    ...
    public void RegisterPivot(int SessionId, String Target) {
        ...
        Database.SaveCommandLog(SessionId, OperatorName != null ? OperatorName : "cli", "pivot " + Target, "Pivot route set: " + Target, true);
        Logger.Custom("  %sPivot route registered: session-%d > %s%s%n", AnsiColor.Green, SessionId, Target, AnsiColor.Reset);
        Logger.Custom("  %s  Use your agent to initiate connection to %s:%d%s%n", AnsiColor.White, PivotHost, PivotPort, AnsiColor.Reset);
    }
